...
class LLMFactoryHelper {
  ...
  static BaseLLMClient createFromModel(llm_model.Model currentModel) {
    try {
      ...
      Logger.root.severe(
          '디버그 - 사용 가능한 API 설정: ${apiSettings.map((s) => "${s.providerId}:${s.apiEndpoint}").join(", ")}');

      final setting = apiSettings.firstWhere(
          (element) => element.providerId == currentModel.providerId,
          orElse: () {
        Logger.root.severe(
            '디버그 - providerId로 일치하는 설정을 찾을 수 없음: ${currentModel.providerId}');
        return apiSettings.firstWhere(
            (element) => element.providerName == currentModel.providerName,
            orElse: () {
          Logger.root.severe(
              '디버그 - providerName으로도 일치하는 설정을 찾을 수 없음: ${currentModel.providerName}');
      ...
      if (provider == null) {
        Logger.root.severe(
            '디버그 - providerId에 해당하는 프로바이더를 찾을 수 없음, apiStyle 사용: ${currentModel.apiStyle}');
      }
